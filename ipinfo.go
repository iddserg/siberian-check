...
// IPInfo holds coarse ISP data about a tested IP, fetched from the free
...
// lookupIPInfo fetches ISP info for ip. Best-effort: network errors or a
...
func lookupIPInfo(ctx context.Context, ip string, timeout time.Duration) (*IPInfo, error) {
	...
	url := fmt.Sprintf("http://ip-api.com/json/%s?fields=status,message,country,isp,org,as,query", ip)
